  ...
  (define (raise-into s t) 
    (let ((s-type (type-tag s)) 
               ...
               (cond ((equal? s-type t-type) s) 
                     ((get 'raise s-type)  
                      (raise-into ((get 'raise s-type) s) t)) 
                     (else #f)))) 
          ...
          (if (= (length args) 2) 
              (let ((a1 (car args)) 
                ...
                (cond  
                  ...
                  (else (error "No method for these types 1 -- APPLY-GENERIC" 
                               (list op type-tags))))) 
              (error "No method for these types 2 -- APPLY-GENERIC" 
                     (list op type-tags))))))) 


(define (numer x) ((get 'rational 'numer) x))
(define (denom x) ((get 'rational 'denom) x))

(define (display-obj o)
  (let ((type (type-tag o)))
    (cond ((eq? type 'integer) (displayln (list 'integer (contents o))))
          ((eq? type 'rational) (displayln (list 'rational (numer o) (denom o))))
          ((eq? type 'complex) (displayln (list 'complex (real-part (contents o)) (imag-part (contents o)))))
          ((eq? type 'real) (displayln (list 'real (contents o))))
          (else (error "Unknown tag -- DISPLAY-OBJ" o)))))
